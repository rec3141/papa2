"""
setup.py — builds libdada2.so via make before packaging.
...
Makefile (which handles compiler flags, OpenMP, zlib, etc.).  Instead we:
  ...
  1. Run ``make libdada2.so`` in the build_ext step so the .so is always
     freshly compiled before any wheel/sdist is assembled.
  2. Copy the resulting libdada2.so into the papa2/ sub-package directory so
...
def _build_shared_library():
    """Run the project Makefile to (re)compile libdada2.so."""
